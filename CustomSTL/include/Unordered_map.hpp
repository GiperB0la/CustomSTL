...
namespace custom_std
{
	...
	private:
		...
		int sizeValue;
		int bucketValue;
		custom_std::Vector<custom_std::Forward_list<custom_std::Vector<custom_std::String>>> bucket;
	};
	...
	void Unordered_map<T1, T2>::inBucket(bool flag)
	{
		...
		custom_std::Vector<custom_std::Vector<custom_std::String>> vec;
		for (custom_std::Forward_list<custom_std::Vector<custom_std::String>> n : bucket) {
			for (custom_std::Vector<custom_std::String> h : n) {
				vec.push_back(h);
			}
		}
		...
		for (custom_std::Vector<custom_std::String> n : vec) {
			insert(n.at(0), custom_std::stoi(n.at(1)));
		}
	}
	...
	{
		if (!find(key)) {
			custom_std::Vector<custom_std::String> bucketVec;
			bucketVec.push_back(key);
			bucketVec.push_back(custom_std::to_string(meaning));
	...
	{
		for (custom_std::Vector<custom_std::String> n : bucket[hash(key)]) {
			for (custom_std::String h : n) {
				if (h == key)
					return 1;
			}
		}
