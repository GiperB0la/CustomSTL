...
namespace custom_std
{
	...
	private:
		custom_std::Vector<custom_std::Vector<T>> blocks;
		int sizeValue;
	};
	...
	void Deque<T>::push_front(T value)
	{
		if (blocks.at(0).size() == 4) {
			custom_std::Vector<custom_std::Vector<T>> newBlocks(blocks.size() + 1);
			int i = 0;
			for (custom_std::Vector<T> block : blocks) {
	...
	void Deque<T>::pop_front()
	{
		...
		if (blocks.at(0).empty()) {
			custom_std::Vector<custom_std::Vector<T>> newBlocks;
			for (custom_std::Vector<T> block : blocks) {
				if (flag) {
					flag = false;
					continue;
	...
	void Deque<T>::clear()
	{
		custom_std::Vector<custom_std::Vector<T>> newBlocks(1);
		blocks = std::move(newBlocks);
		sizeValue = 0;
	}
}